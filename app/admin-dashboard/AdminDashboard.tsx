...
export default function AdminDashboard() {
    ...
    openDialog(
      <div className="space-y-4" dir='rtl'>
        ...
        <Button onClick={async () => {
          ...
          const price = parseInt((document.getElementById('foodPrice') as HTMLInputElement).value)
  ...
  const handleEditFood = useCallback((food: Food) => {
    openDialog(
      <div className="space-y-4" dir='rtl'>
        ...
        <Label htmlFor="foodImage">تصویر غذا</Label>
        <Input id="foodImage" type="file" accept="image/*" />
        <Image src={food.image_url} alt={food.name} width={100} height={100} className="mt-2 rounded-md" />
        <Button onClick={async () => {
          const name = (document.getElementById('food') as HTMLInputElement).value
          const description = (document.getElementById('foodDescription') as HTMLInputElement).value
          const price = parseInt((document.getElementById('foodPrice') as HTMLInputElement).value)
    ...
    openDialog(
      <form onSubmit={async (e) => {
        ...
        const newItem = {
          ...
          start_time,
          end_time,
          time_slot_count: parseInt(time_slot_count),
          time_slot_capacity: parseInt(time_slot_capacity),
        ...
        <Button type="submit" className="w-full">افزودن غذا به منو</Button>
      </form>
    )
  ...
  const handleEditMenuItem = useCallback((item: MenuItemSpec, isTemplate: boolean) => {
    openDialog(
      <form onSubmit={async (e) => {
        e.preventDefault()
        const formData = new FormData(e.currentTarget)
        ...
        <Label htmlFor="time_slot_count">تعداد بازه‌های زمانی</Label>
        <Input id="time_slot_count" name="time_slot_count" type="number" defaultValue={item.time_slot_count} required min="1" />
        <Label htmlFor="time_slot_capacity">ظرفیت هر بازه زمانی</Label>
        <Input id="time_slot_capacity" name="time_slot_capacity" type="number" defaultValue={item.time_slot_capacity} required min="1" />
        <Label htmlFor="daily_capacity">ظرفیت روزانه</Label>
        <Input id="daily_capacity" name="daily_capacity" type="number" defaultValue={item.daily_capacity} required min="1" />
        <Button type="submit" className="w-full">ذخیره تغییرات</Button>
      </form>
    )
  }, [foods, openDialog, closeDialog, fetchTemplateMenu, fetchDailyMenu])

  const handleDeleteMenuItem = useCallback(async (itemId: string, isTemplate: boolean) => {
    try {
      if (isTemplate) {
  ...
  const handleUpdateVoucherPrice = useCallback(() => {
    openDialog(
      <div className="space-y-4" dir='rtl'>
        ...
        <Button onClick={async () => {
          const newPrice = parseInt((document.getElementById('voucherPrice') as HTMLInputElement).value)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-bl from-orange-100 to-red-100 rtl p-8">
      ...
      <header className="sticky top-0 z-50 w-full bg-white shadow-md mb-8">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          <Link href="/" className="flex items-center space-x-2">
            ...
            <span className="text-xl font-bold text-orange-500">سامانه رزرو غذای دانشگاه - پنل مدیریت</span>
          </Link>
          <TooltipProvider>
            <Tooltip>
      ...
      <main className="container mx-auto px-4 pb-8">
        ...
        >
          <h1 className="text-4xl font-bold text-orange-500 mb-2">پنل مدیریت</h1>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex justify-between items-center">
                  <span>مدیریت غذاها</span>
                  <Button onClick={handleAddFood}><Plus className="w-4 h-4 mr-2" /> افزودن غذا</Button>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <Table>
                  ...
                  <TableBody>
                    {foods.map((food) => (
                      <TableRow key={food.id}>
                        <TableCell>
                          <Image src={food.image_url} alt={food.name} width={50} height={50} className="rounded-md" />
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="flex space-x-2">
                            <Button variant="outline" size="sm" onClick={() => handleEditFood(food)}><Edit className="w-4 h-4" /></Button>
                            <Button variant="destructive" size="sm" onClick={() => handleDeleteFood(food)}><Trash className="w-4 h-4" /></Button>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))}
              ...
              <CardHeader>
                ...
                <CardContent>
                  <div className="flex space-x-4 mb-4">
                    <Select value={selectedDay} onValueChange={setSelectedDay}>
                      ...
                      <SelectContent>
                        {[
                          ...
                          <SelectItem key={day.en} value={day.en}>{day.fa}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                    <Select value={selectedMeal_type} onValueChange={(value: 'lunch' | 'dinner') => setSelectedMeal_type(value)}>
                      <SelectTrigger className="w-[180px]">
                    ...
                    <Button onClick={() => handleAddMenuItem(true)}>افزودن غذا به منو</Button>
                  </div>
                  <Table>
                    <TableHeader>
                      <TableRow>
                            ...
                            <TableCell>
                              {item.items.map((menuItem, itemIndex) => (
                                <div key={itemIndex} className="flex space-x-2 mb-2">
                                  <Button variant="outline" size="sm" onClick={() => handleEditMenuItem(menuItem, true)}><Edit className="w-4 h-4" /></Button>
                                  <Button variant="destructive" size="sm" onClick={() => handleDeleteMenuItem(menuItem.id, true)}><Trash className="w-4 h-4" /></Button>
                                </div>
                              ))}
                            </TableCell>
                          </TableRow>
            ...
            <TabsContent value="daily-menu">
                ...
                <CardHeader>
                  ...
                  <CardContent>
                    <div className="flex space-x-4 mb-4">
                      ...
                      <Button onClick={() => handleAddMenuItem(false)}>افزودن غذا به منو</Button>
                      <Button onClick={handleUseTemplateForDaily}>استفاده از منوی الگو</Button>
                    </div>
                    <Table>
                      <TableHeader>
                        <TableRow>
                            ...
                            <TableRow key={index}>
                              ...
                              <TableCell>
                                  <div key={index} className="flex space-x-2 mb-2">
                                    <Button variant="outline" size="sm" onClick={() => handleEditMenuItem(item, false)}><Edit className="w-4 h-4" /></Button>
                                    <Button variant="destructive" size="sm" onClick={() => handleDeleteMenuItem(item.id, false)}><Trash className="w-4 h-4" /></Button>
                                  </div>
                              </TableCell>
                            </TableRow>
                          ))}
                  ...
                  <CardHeader>
                    ...
                    <CardContent>
                      <div className="flex justify-between items-center">
                        ...
                        <Button onClick={handleUpdateVoucherPrice}>به‌روزرسانی قیمت</Button>
                      </div>
                    </CardContent>
                  </Card>
                </TabsContent>
                            ...
                            <div className="w-full h-[400px]">
                              <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={dailyOrderCounts}>
                                  ...
                                  <RechartsTooltip />
                                  <Legend />
                                  <Bar dataKey="count" fill="#8884d8" name="تعداد سفارشات" />
                                </BarChart>
                              </ResponsiveContainer>
                            </div>
                          </TabsContent>
              ...
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="sm:max-w-[425px]">
                  {dialogContent}
                  <DialogFooter>
                    <Button variant="outline" onClick={closeDialog}>بستن</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            </div>
